...
- **555 訓練**：賽道一致性訓練（基準→設超越目標→超越→設達標目標→達標），round 2
  與 round 3 都可自訂進步目標時間；即時專注畫面 + 完成計分，可**暫停續傳**。
  訓練法概念源自賽車一丁的影片
  [《不養成好習慣, 就是在培養一個壞習慣》](https://youtu.be/_JJEQIiwphw)。
- **AI 教練**：自動組裝賽道知識 + 兩圈分析摘要當 context，逐字串流回覆、對話自動存檔。
  **可選供應商**：Anthropic（Claude）/ OpenAI / Google（Gemini）/ 本地或自訂
  OpenAI 相容端點（Ollama、LM Studio…），模型自由填。

### AI 教練設定

點左上 ⚙ → 選供應商 → 填 API 金鑰與模型 → 測試連線。各家金鑰分開存於本機
`config.json`（已 gitignore），即存即用。本地供應商填 Base URL（預設 Ollama
`http://localhost:11434/v1`）、金鑰通常留空。也支援環境變數
`COACH_PROVIDER` / `COACH_MODEL` / `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GOOGLE_API_KEY`。

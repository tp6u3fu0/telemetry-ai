...
按「開始」後進入**專注畫面**：特大的當前圈用時、已完成圈數、5 圈進度點、
圈速清單（失誤圈紅色標示）——簡潔到適合邊開邊瞄。到設定目標階段會出現輸入框，
完成後顯示總分。「停止 / 返回首頁」離開。

## AI 教練回覆採串流

教練回覆改為**逐字串流**——1–2 秒就開始出字，不再乾等十幾秒像卡住。
（回覆完成後照常存進對話紀錄。）
...
深色賽車儀表板：左側「檢視車手」下拉（對手依最快圈排序），選車手後其圈成為
可拖曳卡片；把卡片**拖到右上的 A / B 比較欄**，下方儀表板即時重算。
**B 欄留空＝單圈分析**：速度/踏板/方向盤/檔位軌跡、車速著色的賽道地圖、單圈煞車
區段表，AI 教練也支援單圈。⇄ 交換 A/B、✕ 清除、雙擊卡片快速指派。比較模式顯示——
...
telemetry-ai.spec           # PyInstaller 打包設定（onefile, Windows）
```
...
並把 `Telemetry-AI.exe` 附到對應的 Release：
